            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                @if ($organization->show_meetings_pane)
                    ...
                    <div class="{{ $paneCardClass }} @if($expandLast && $lastPane === 'meetings') lg:col-span-2 @endif">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="{{ $paneIconBadge }} bg-leather-50 dark:bg-leather-700/30 text-leather-500 dark:text-leather-300">
                            ...
                            <h3 class="text-sm font-semibold text-ink-800 dark:text-paper-100">{{ __('今後の会議予定') }}</h3>
                        </div>
                        <ul class="space-y-3">
                ...
                @if ($organization->show_calendar_pane)
                    ...
                    <div class="{{ $paneCardClass }} @if($expandLast && $lastPane === 'calendar') lg:col-span-2 @endif">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="{{ $paneIconBadge }} bg-ink-50 dark:bg-ink-700/40 text-ink-600 dark:text-paper-200">
                                <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <rect x="4" y="5" width="16" height="15" rx="2" />
                                    <path d="M4 10h16M8 3v4M16 3v4" />
                                </svg>
                            </span>
                            <h3 class="text-sm font-semibold text-ink-800 dark:text-paper-100">{{ __('カレンダー') }}</h3>
                        </div>
                        @if ($organization->google_calendar_id)
                            <iframe
                                ...
                                class="w-full h-64 border-0 rounded-md" loading="lazy"></iframe>
                        @else
                ...
                @if ($organization->show_birthday_pane)
                    ...
                    <div class="{{ $paneCardClass }} @if($expandLast && $lastPane === 'birthday') lg:col-span-2 @endif">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="{{ $paneIconBadge }} bg-brass-100 dark:bg-brass-600/25 text-brass-600 dark:text-brass-300">
                                <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            ...
                            <h3 class="text-sm font-semibold text-ink-800 dark:text-paper-100">{{ __('今月の誕生日') }}</h3>
                        </div>
                        <ul class="space-y-2">
                            @forelse ($birthdayMembers as $member)
                                @php $isToday = $member->birth_date->day === now()->day; @endphp
                                <li class="flex items-center gap-3 text-sm @if($isToday) -mx-2 rounded-md bg-brass-100 dark:bg-brass-600/20 px-2 py-1.5 @endif">
                                    @if ($member->photoUrl())
                ...
                @if ($organization->show_materials_pane)
                    ...
                    <div class="{{ $paneCardClass }} @if($expandLast && $lastPane === 'materials') lg:col-span-2 @endif">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h3 class="text-sm font-semibold text-ink-800 dark:text-paper-100">{{ __('資料置き場') }}</h3>
                        </div>
                        <ul class="space-y-2">
