@props(['label' => 'スクリーンショット', 'url' => 'your-agenda.example/dashboard', 'src' => null])

{{-- srcを渡すと実画面のスクリーンショットを表示する。渡さなければ、まだ
     用意できていない画面用のプレースホルダーになる。どちらもブラウザ風の
     フレームに入れ、実在する製品であることを印象づける。 --}}
<div {{ $attributes->merge(['class' => 'rounded-xl overflow-hidden shadow-2xl border border-paper-200 dark:border-ink-700 bg-paper-50 dark:bg-ink-800']) }}>
    ...
    @if ($src)
        <img src="{{ $src }}" alt="{{ $label }}" class="aspect-[16/10] w-full object-cover object-top" loading="lazy">
    @else
