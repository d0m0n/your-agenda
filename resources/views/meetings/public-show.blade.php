...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-paper-100 dark:bg-night text-ink-800 dark:text-paper-100 print:bg-white">

        {{-- 簡易ヘッダー: ログイン画面等へのナビはなく、ブランドと印刷ボタンのみ --}}
        <header class="print-hidden border-b border-paper-200 dark:border-ink-700">
                                ...
                                <li class="border-b border-paper-200 dark:border-ink-700 pb-4 last:border-b-0 last:pb-0">
                                    <div class="flex items-center justify-between gap-4">
                                        <p class="text-sm font-medium text-ink-800 dark:text-paper-100">
                                            ...
                                            @if ($item->publicLinkUrl())
                                                <a href="{{ $item->publicLinkUrl() }}" target="_blank" rel="noopener noreferrer" class="text-leather-500 dark:text-leather-300 hover:underline">{{ $item->title }}</a>
                                            @else
                                                {{ $item->title }}
                                            @endif
                                        </p>
                                            ...
                                            @foreach ($item->children as $childIndex => $child)
                                                <li class="flex items-center justify-between gap-4">
                                                    <p class="text-sm text-ink-700 dark:text-paper-200">
                                                        ...
                                                        @if ($child->publicLinkUrl())
                                                            <a href="{{ $child->publicLinkUrl() }}" target="_blank" rel="noopener noreferrer" class="text-leather-500 dark:text-leather-300 hover:underline">{{ $child->title }}</a>
                                                        @else
                                                            {{ $child->title }}
                                                        @endif
                                                    </p>
